...
```sh
$ git clone git://github.com/cloway/telcong.bss
$ cd telcong.bss/frontend
...
Note that build.sh is running the following commands in parallel: 
...
Finally, open `http://localhost:8090/` in your browser.
...
getting `frontend` up and running. Though `frontend` is really simple
...
`frontend` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `frontend`, like Twitter Bootstrap
...
Technically, `frontend` is now ready to go.
...
`frontend`. What follows in this section is a quick introduction to the
...
See the [issues list](http://github.com/clouway/telcong.bss/issues). And feel free to submit your own!
